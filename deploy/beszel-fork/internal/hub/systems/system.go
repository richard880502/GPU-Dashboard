...
func createContainerRecords(app core.App, data []*container.Stats, systemId string) error {
	...
	for i, container := range data {
		suffix := fmt.Sprintf("%d", i)
		valueStrings = append(valueStrings, fmt.Sprintf("({:id%[1]s}, {:system}, {:name%[1]s}, {:image%[1]s}, {:ports%[1]s}, {:status%[1]s}, {:health%[1]s}, {:cpu%[1]s}, {:memory%[1]s}, {:net%[1]s}, {:updateAvailable%[1]s}, {:gpuPid%[1]s}, {:gpuMemMiB%[1]s}, {:gpuMemPercent%[1]s}, {:gpuUtilPercent%[1]s}, {:gpuIndex%[1]s}, {:updated})", suffix))
		...
		params["gpuPid"+suffix] = container.GpuPid
		params["gpuMemMiB"+suffix] = container.GpuMemMiB
		params["gpuMemPercent"+suffix] = container.GpuMemPercent
		params["gpuUtilPercent"+suffix] = container.GpuUtilPercent
		params["gpuIndex"+suffix] = container.GpuIndex
	}
	queryString := fmt.Sprintf(
		"INSERT INTO containers (id, system, name, image, ports, status, health, cpu, memory, net, updatable, gpuPid, gpuMemMiB, gpuMemPercent, gpuUtilPercent, gpuIndex, updated) VALUES %s ON CONFLICT(id) DO UPDATE SET system = excluded.system, name = excluded.name, image = excluded.image, ports = excluded.ports, status = excluded.status, health = excluded.health, cpu = excluded.cpu, memory = excluded.memory, net = excluded.net, updatable = excluded.updatable, gpuPid = excluded.gpuPid, gpuMemMiB = excluded.gpuMemMiB, gpuMemPercent = excluded.gpuMemPercent, gpuUtilPercent = excluded.gpuUtilPercent, gpuIndex = excluded.gpuIndex, updated = excluded.updated",
		strings.Join(valueStrings, ","),
	)
