...
import { cn, decimalString, formatBytes, getServerDotColor, hourWithSeconds } from "@/lib/utils"
...
export const containerChartCols: ColumnDef<ContainerRecord>[] = [
				...
				<div className="ms-1 flex items-center gap-1.5">
					<span
						className={cn(
							...
							getServerDotColor(
								name,
								Object.values(allSystems).map((s) => s.name)
							)
						)}
					/>
	...
	// gpu-monitoring fork addition (not upstream): surfaces per-container GPU
	// usage (from gpu-process-exporter + nvitop-exporter) so it's visible at
	// a glance which containers are using which GPU, and how much.
	{
		...
		cell: ({ row }) => {
			const mib = row.original.gpuMemMiB ?? 0
			const formatted = formatBytes(mib, false, undefined, true)
			const pct = row.original.gpuMemPercent ?? 0
			return (
				<span className="ms-1 tabular-nums">
					{`${decimalString(formatted.value, formatted.value >= 10 ? 1 : 2)} ${formatted.unit} (${decimalString(pct, pct >= 10 ? 1 : 2)}%)`}
				</span>
			)
		},
	},
	...
	},
	{
		...
		cell: ({ getValue }) => {
			...
			return <span className="ms-1.5 tabular-nums">{val ?? <span className="text-muted-foreground">-</span>}</span>
		},
	},
	{
		id: "net",
